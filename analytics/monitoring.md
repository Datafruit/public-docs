...
监控告警是日志运维中最重要功能，可以轻松替您监控数据，我们可以把您关注的指标按预设计划周期性执行，当满足触发条件时我们会及时通知您。
...
* 告警方式：配置收到告警的方式，支持短信告警、邮件告警和接口告警三种方式。同时支持点击测试发送是否成功
* 设定告警规则：配置发生异常时，发送通知信息的规律。提供三种规律进行选择：
...
为了达到上述的目的，需要对错误码的命名及使用进行统一的管理，并将含有错误码的后台日志实时推送到平台进行分析处理。
